...
void spatialJoin() {
    ...
    for (itr = report_stat.begin(); itr != report_stat.end(); ++itr) {
        ...
        vector<string> boundry = split(tile_str[itr->first], ' ');        
        string head = "{\"tile_id\":" + std::to_string(static_cast<unsigned long long>(itr->first)) +"," +
                     "\"tile_size\":" + std::to_string(static_cast<unsigned long long>(tile_size)) + "," +
                     "\"tile_boundary\":{\"min_x\":" + boundry[0] +",\"min_y\":" + boundry[1] +",\"max_x\":" + boundry[2] + ",\"max_y\":" + boundry[3] +"},";
        string s1 = head + "\"matrix_type\":\"tile_dice\"," + "\"matrix_value\":" + std::to_string(static_cast<long double>((itr->second)[0])) + "}\n";
        string s2 = head + "\"matrix_type\":\"jacc\"," + "\"matrix_value\":" + std::to_string(static_cast<long double>((itr->second)[1])) + "}\n";
        string s3 = head + "\"matrix_type\":\"dice\"," + "\"matrix_value\":" + std::to_string(static_cast<long double>((itr->second)[2])) + "}\n";
        //cout << s1; cout << s2; cout << s3;
        std::ofstream out1(writePath+"/tile_dice_"+std::to_string(static_cast<unsigned long long>(itr->first))+".json");
        std::ofstream out2(writePath+"/jacc_"+std::to_string(static_cast<unsigned long long>(itr->first))+".json");
        std::ofstream out3(writePath+"/dice_"+std::to_string(static_cast<unsigned long long>(itr->first))+".json");
        out1 << s1;  out2 << s2;  out3 << s3;
        out1.close();
    }
}
...
int main (int argc, char **argv){   
    ...
    writePath = std::string(argv[4], strlen(argv[4]));
    genTiles(tile_size);
    if (!buildIndex()) {
    ...
    std::cerr << "Done. Heatmaps are stored at " << writePath << ".\n"; 
    return 0;
}
